...
export const nuevoRole = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { descripcion, negocio } = req.body;

    // Buscamos que no exista una descripcion igual en el mismo restaurante
    const existeDescripcion = await Role.findOne({ descripcion: descripcion, negocio });
